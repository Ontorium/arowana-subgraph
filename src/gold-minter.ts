...
function getOrCreateUserMintStats(
    address: Bytes,
    timestamp: BigInt,
): UserMintStats {
...
export function handleSettleMint(event: SettleMintEvent): void {
    ...
    if (mintRequest != null) {
        ...
        if (event.params.success) {
            ...
            let userStats = getOrCreateUserMintStats(
                mintRequest.buyer,
                event.block.timestamp,
            );
            userStats.mintCount = userStats.mintCount + 1;
            userStats.totalMintVolume =
                userStats.totalMintVolume.plus(netGoldAmount);
...
export function handleSettleBurn(event: SettleBurnEvent): void {
    ...
    if (redeemRequest != null) {
        ...
        if (event.params.success) {
            let stats = getOrCreateMintStats();
            stats.totalRedeemCount = stats.totalRedeemCount.plus(
                BigInt.fromI32(1),
            );
            stats.totalRedeemVolume = stats.totalRedeemVolume.plus(
                redeemRequest.goldAmount,
            );
            stats.save();

            let userStats = getOrCreateUserMintStats(
                redeemRequest.seller,
                event.block.timestamp,
            );
            userStats.redeemCount = userStats.redeemCount + 1;
            userStats.totalRedeemVolume = userStats.totalRedeemVolume.plus(
                redeemRequest.goldAmount,
            );
            userStats.lastActivityAt = event.block.timestamp;
            userStats.save();
